...
interpretation: In the log product calculation,
...
of 5000 data points. The distribution plot of r.squared is almost
normally distributed between 0.86 to 0.96, with the center around 0.92.
The distribution plot of log products, however, is clearly not normally
distributed, it is left-skewed and has a heavy tail extending to low
values, values around -6 has the highest density. Features like that may
be related to the frequency with with large outliers are included in the
bootstrap sample.
...
bwt_df |>
  ...
  ylab("Residual") +xlab("Fitted values") +labs(title = "plot of resitual against fitted values")
```

<img src="p8105_hw6_lz2949_files/figure-gfm/unnamed-chunk-10-1.png" width="90%" />
Description: The idea that mothers who experience a more substantial
weight gain during prenancy are likely to deliver babies with higher
birth weights immediately comes to my mind. I first plot the baby
birthweight vs. mother’s weight gain during pregnancy and fit a line to
see the trend, the line has a positive slope, indicating that there is a
positive correlation between mother’s weight gain and birth weight of
baby. Then I build the slr. <br> From the residual plot, it can be found
that most points are gathered around 0 with no obvious trend or
distribution.
...
b_df = 
  ...
  mutate(
    rmse_prop_linear = map2_dbl(prop_lm, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_lbga_linear = map2_dbl(lbga_mod, test, \(mod, df) rmse(model = mod, data = df)),
    rmse_hcls_smooth = map2_dbl(hcls_mod, test, \(mod, df) rmse(model = mod, data = df)))
```
...
<img src="p8105_hw6_lz2949_files/figure-gfm/unnamed-chunk-13-1.png" width="90%" />

Interpretation: From the violin plot, it can be see that the second and
third models are better than the first one, they have lower rmse values,
indicating they have smaller prediction error. Probabily because the
first model only uses 1 factor as the predictor and the second and third
use more complex model than the first one which will fit data in a
better way.

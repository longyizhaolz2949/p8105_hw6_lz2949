...
```
interpretation: Lots of Beta2 are negative values, leading to log(negative numbers) which does not exist in real number space. Among 5000 log products, `r estimated_data |> filter(log_product == "NaN") |> count()` of them are NaN. Which counts for `r (estimated_data |> filter(log_product == "NaN") |> count())/5000*100` percent of 5000 data points. The distribution plot of r.squared is almost normally distributed between 0.86 to 0.96, with the center around 0.92. The distribution plot of log products, however, is clearly not normally distributed, it is left-skewed and has a heavy tail extending to low values, values around -6 has the highest density. Features like that may be related to the frequency with with large outliers are included in the bootstrap sample. 
...
```
Description: The idea that mothers who experience a more substantial weight gain during prenancy are likely to deliver babies with higher birth weights immediately comes to my mind. I first plot the baby birthweight vs. mother's weight gain during pregnancy and fit a line to see the trend, the line has a positive slope, indicating that there is a positive correlation between mother's weight gain and birth weight of baby. Then I build the slr. 
<br>
...
Interpretation: From the violin plot, it can be see that the second and third models are better than the first one, they have lower rmse values, indicating they have smaller prediction error. Probabily because the first model only uses 1 factor as the predictor and the second and third use more complex model than the first one which will fit data in a better way. 
